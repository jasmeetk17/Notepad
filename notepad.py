...
file.add_command(label="Exit",accelerator="ctrl+e",image=exit,compound="left")
...
view.add_command(label="Status Bar",accelerator="ctrl+shift+s",image=statusbar,compound="left")
view.add_command(label="Tool Bar",accelerator="ctrl+shift+t",image=toolbar,compound="left")
...
'''
font_values = tk.font.families
print(str(font_values))
'''

#create toll menu
tool_bar_menu = tk.Menu(menu_bar,tearoff=False)
...
def open_tool_bar():
    ...
    tool_bar.place(x=150,y=2)

menu_bar.add_cascade(label="ToolBar",menu=tool_bar_menu)
tool_bar_menu.add_command(label="Open Tools For Font",command=open_tool_bar)
